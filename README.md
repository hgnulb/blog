...
[![Phosphor Icons](https://img.shields.io/badge/Phosphor_Icons-35313D?style=flat)](https://phosphoricons.com/)
...
| 命令                             | 说明                           |
|----------------------------------|--------------------------------|
| `make install`                   | 安装 Node、Ruby、Python 依赖   |
| `make dev`                       | 启动本地开发服务               |
| `make build`                     | 执行生产构建                   |
| `make format`                    | 格式化代码                     |
| `make create-article-template`   | 创建文章模板                   |
| `make generate-leetcode-article` | 批量生成 LeetCode 题解         |
| `make get-codetop-data`          | 获取 CodeTop 数据              |
| `make generate-emoji-scss`       | 生成 Emoji SCSS                |
| `make check-duplicates`          | 检查重复文章                   |
| `make set-published`             | 批量设置题库发布状态，跳过笔记 |
| `make cleanup-workflow`          | 清理工作流记录                 |
| `make cleanup-unused-image`      | 清理未引用图片                 |
| `make push`                      | 提交并推送                     |
| `make cleanup`                   | 清理提交历史并强制推送         |
| `make notify-deploy`             | 发送部署通知                   |
